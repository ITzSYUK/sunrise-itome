...
const { data } = await useAuto(best_category, 'Лучшее', 'korea')
...
const { data: top } = await useAuto(top_category, 'ТОП', 'korea')
...
<template>
  ...
  >
    ...
    <template #description>
      Наши специалисты в Корее снимают детальный видеообзор авто на этапе <br> просмотра. Поможем с выбором, купим, привезём, доставим в любой город РФ.
    </template>
  ...
  <RunningString class="mt-150px" :strings="['HYUNDAI', 'KIA', 'GENESIS', 'SSANGYONG', 'DAEWOO', 'SSANGYONG']" />
...
актуальную стоимость автомобиля и свяжемся с вами."
    image-src="free_consultation_2.webp"
  />
